...
describe('eventsRoutes', () => {
  ...
  it('GET /namespace/:namespace/events returns events', async () => {
    mockListInNamespace.mockResolvedValue({ items: [{ type: 'Normal', reason: 'Started' }] });
    const response = await fastify.inject({ method: 'GET', url: '/namespace/admin-che/events' });
